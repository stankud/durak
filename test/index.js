...
import updateGameState from '../src/server/update-game-state'
...
/* *** updateLegalMoves() tests *** */
test('updateLegalMoves() initialUpdate', (t) => {
  const gameState = {
  ...
  const { ngs: actualGameState } = dealer.updateLegalMoves({ cgs: gameState });
  t.ok(actualGameState.players[0].legalMoves, 'player 1 has legalMoves prop');
  t.ok(
    actualGameState.players[0].legalMoves[0] === 'attack',
    'player 1 has "attack" in legalMoves'
  );
  t.ok(actualGameState.players[1].legalMoves, 'player 2 has legalMoves prop');
  t.ok(
    actualGameState.players[1].legalMoves.length === 0,
    'player 2 has no legalMoves'
  );
  t.ok(actualGameState.players[2].legalMoves, 'player 3 has legalMoves prop');
  t.ok(
    actualGameState.players[2].legalMoves.length === 0,
    'player 3 has no legalMoves'
  );
  t.ok(actualGameState.players[3].legalMoves, 'player 4 has legalMoves prop');
  t.ok(
    actualGameState.players[3].legalMoves.length === 0,
    'player 4 has no legalMoves'
  );
  t.deepEqual(
    actualGameState.players[0].legalMoves, expected[0],
    'player 1 has correct legalMoves'
  );
  t.deepEqual(
    actualGameState.players[1].legalMoves,
    expected[1], 'player 2 has correct legalMoves'
  );
  t.deepEqual(
    actualGameState.players[2].legalMoves,
    expected[2], 'player 3 has correct legalMoves'
  );
  t.deepEqual(
    actualGameState.players[3].legalMoves,
    expected[3], 'player 4 has correct legalMoves'
  );
  t.end();
});

test.only('updateLegalMoves() attackStarted', (t) => {
  const currentState = {
  ...
  const expected = [['attack'], [], [], []];
  const actualGameState = dealer.updateLegalMoves({ 
    cgs: currentState,
    nps: playerState
  });
  t.ok(actualGameState.players[0].legalMoves, 'player 1 has legalMoves prop');
...
/* *** returnPlayerGameState() tests *** */
test('returnPlayerGameState()', (t) => {
  const gameState = {
    deck: ['6H','JC','AH','6D','6S','TC','AD','TD','KC','8D','7C','8S'],
    cardsOffense: ['6C'],
    cardsDefense: [],
...
test('isValidStateUpdate() when no update is made', (t) => {
  ...
  t.is(result, 'NoUpdate', 'correct result');
  t.is(move, undefined, 'correct move');
  t.end();
});
...
test('isValidStateUpdate() illegal end-attack move is made', (t) => {
  ...
  const actual = dealer.isValidStateUpdate({ gameState, playerState });
  const { result, move } = actual;
  t.is(result, 'IllegalMove', 'correct result');
  t.is(move, 'end-attack', 'correct move');
  t.end();
});
...
test('isValidStateUpdate() an invalid throw-in move is made', (t) => {
  ...
  const actual = dealer.isValidStateUpdate({ gameState, playerState });
  const { result, move } = actual;
  t.is(result, 'InvalidMove', 'correct result');
  t.is(move, 'throw-in', 'correct move');
  t.end();
});

test('isValidStateUpdate() a legal and valid throw-in move is made', (t) => {
  const gameState = {
...
test('isValidStateUpdate() an illegal attack move is made', (t) => {
  ...
  const { result, move } = actual;
  t.is(result, 'IllegalMove', 'correct result');
  t.is(move, 'attack', 'correct move');
  t.end();
});

test('isValidStateUpdate() a legal attack move is made', (t) => {
  const gameState = {
    deck: ['6H','JC','AH','6D','6S','TC','AD','TD','KC','8D','7C','8S'],
    cardsOffense: [],
    cardsDefense: [],
  ...
  const playerState = {
    playerId: 'id1',
    cardsPlayer: ['KS','JH','JD','7D','7H'],
    cardsDeckCount: 12,
    playerLegalMoves: ['attack'],
    cardsOffense: ['6C'],
    cardsDefense: [],
    trump: '6H',
    cardsOpponentsCounts: [6, 6, 6]
...
test('isValidStateUpdate() an illegal defend move is made', (t) => {
  const gameState = {
    deck: ['6H','JC','AH','6D','6S','TC','AD','TD','KC','8D','7C','8S'],
    cardsOffense: ['6C'],
    cardsDefense: [],
  ...
  const { result, move } = actual;
  t.is(result, 'IllegalMove', 'correct result');
  t.is(move, 'defend', 'correct move');
  t.end();
});

test('isValidStateUpdate() an invalid defend move is made', (t) => {
  const gameState = {
    deck: ['6H','JC','AH','6D','6S','TC','AD','TD','KC','8D','7C','8S'],
    cardsOffense: ['6C'],
    cardsDefense: [],
  ...
  const playerState = {
    playerId: 'id2',
    cardsPlayer: ['KH','TH','9C','QH','8C'],
    cardsDeckCount: 12,
    playerLegalMoves: ['defend'],
    cardsOffense: ['6C'],
    cardsDefense: ['9D'],
    trump: '6H',
    cardsOpponentsCounts: [6, 6, 5]
...
test('isValidStateUpdate() a legal and valid defend move is made', (t) => {
  const gameState = {
    deck: ['6H','JC','AH','6D','6S','TC','AD','TD','KC','8D','7C','8S'],
    cardsOffense: ['6C'],
    cardsDefense: [],
...
test('isValidStateUpdate() an illegal pick-up move is made', (t) => {
  const gameState = {
    deck: ['6H','JC','AH','6D','6S','TC','AD','TD','KC','8D','7C','8S'],
    cardsOffense: ['6C'],
    cardsDefense: [],
  ...
  const playerState = {
    playerId: 'id3',
    cardsPlayer: ['7S','QC','QD','KD','AC','9S','6C'],
    cardsDeckCount: 12,
    playerLegalMoves: ['throw-in'],
    cardsOffense: [],
    cardsDefense: [],
...
test('isValidStateUpdate() a legal pick-up move is made', (t) => {
  const gameState = {
    deck: ['6H','JC','AH','6D','6S','TC','AD','TD','KC','8D','7C','8S'],
    cardsOffense: ['6C'],
    cardsDefense: [],
  ...
  const playerState = {
    playerId: 'id2',
    cardsPlayer: ['KH','9D','TH','9C','QH','8C','6C'],
    cardsDeckCount: 12,
    playerLegalMoves: ['defend'],
    cardsOffense: [],
    cardsDefense: [],
...
/* *** updateGameState() roundOver tests *** */
test.skip('updateGameState()', (t) => {
  const currentState = {
    deck: ['8S', '9D', 'JS', '7S', 'AS', '6C', 'KC', 'JD', 'JC', '9S', '8C', 'AH'],
    cardsOffense: ['8D','QH'],
    cardsDefense: ['QD','KH'],
